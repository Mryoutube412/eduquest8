...
export default function Results() {
  ...
  const getEmoji = () => { if (percentage >= 90) return '🏆'; if (percentage >= 70) return '🎉'; if (percentage >= 50) return '👍'; return '💪'; };
  const getMessage = () => { if (percentage >= 90) return 'Mükemmel!'; if (percentage >= 70) return 'Çok iyi!'; if (percentage >= 50) return 'İyi gidiyorsun!'; return 'Tekrar dene!'; };
      ...
      <div className="min-h-screen bg-background bg-pattern flex items-center justify-center p-4">
        <div className="w-full max-w-2xl space-y-6 animate-fade-in">
          ...
          <div className="flex gap-3">
            <button onClick={() => setReviewIndex(null)} className="flex-1 bg-card rounded-xl p-4 card-shadow font-medium text-card-foreground hover:scale-[1.02] transition-transform touch-target">← Sonuçlara Dön</button>
            {reviewIndex > 0 && <button onClick={() => setReviewIndex(reviewIndex - 1)} className="bg-card rounded-xl p-4 card-shadow font-medium text-card-foreground hover:scale-[1.02] transition-transform touch-target">◀ Önceki</button>}
            {reviewIndex < total - 1 && <button onClick={() => setReviewIndex(reviewIndex + 1)} className="bg-card rounded-xl p-4 card-shadow font-medium text-card-foreground hover:scale-[1.02] transition-transform touch-target">Sonraki ▶</button>}
          </div>
        </div>
      </div>
    );
      ...
      <div className="w-full max-w-2xl space-y-6 animate-fade-in">
        <div className="bg-card rounded-2xl p-8 card-shadow text-center">
          ...
          <div className="grid grid-cols-3 gap-4 text-center">
            <div className="bg-muted rounded-xl p-3"><p className="text-2xl font-bold text-card-foreground">{total}</p><p className="text-xs text-muted-foreground">Toplam</p></div>
            <div className="bg-success/10 rounded-xl p-3"><p className="text-2xl font-bold text-success">{correct}</p><p className="text-xs text-muted-foreground">Doğru</p></div>
            <div className="bg-destructive/10 rounded-xl p-3"><p className="text-2xl font-bold text-destructive">{total - correct}</p><p className="text-xs text-muted-foreground">Yanlış</p></div>
          </div>
        </div>
        ...
        <div className="bg-card rounded-2xl p-6 card-shadow">
          ...
          <div className="space-y-2">
            {answers.map((a, i) => (
              <button key={i} onClick={() => setReviewIndex(i)} className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-muted transition-colors text-left touch-target">
                <span className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold shrink-0 ${a.correct ? 'bg-success/20 text-success' : 'bg-destructive/20 text-destructive'}`}>{a.correct ? '✓' : '✗'}</span>
          ...
          <div className="bg-destructive/5 border border-destructive/20 rounded-2xl p-6">
            ...
            <div className="space-y-2">
              {wrong.map((a, i) => {
                ...
                return (
                  <button key={i} onClick={() => setReviewIndex(originalIndex)} className="w-full text-left p-3 rounded-xl hover:bg-destructive/10 transition-colors touch-target">
